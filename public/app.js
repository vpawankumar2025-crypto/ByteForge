...
function applyPriceFilter(items, rangeValue){
  if (!rangeValue) return items;
  const [min, max] = rangeValue.split('-').map(Number);
  return items.filter(i => i.price >= min && i.price <= max);
}
function applySort(items, sortValue){
  if (sortValue === 'price-asc') return [...items].sort((a,b) => a.price - b.price);
  if (sortValue === 'price-desc') return [...items].sort((a,b) => b.price - a.price);
  return items;
}
...
function renderCourseList(){
  ...
  const priceF = document.getElementById('courseFilterPrice').value;
  const sortF = document.getElementById('courseSortBy').value;
  ...
  items = applyPriceFilter(items, priceF);
  items = applySort(items, sortF);
...
function renderInternshipList(){
  ...
  const priceF = document.getElementById('internFilterPrice').value;
  const sortF = document.getElementById('internSortBy').value;
...
document.getElementById('courseFilterPrice').addEventListener('change', renderCourseList);
document.getElementById('courseSortBy').addEventListener('change', renderCourseList);
...
document.getElementById('internFilterPrice').addEventListener('change', renderInternshipList);
document.getElementById('internSortBy').addEventListener('change', renderInternshipList);
...
async function fetchFullDetail(id){
  ...
  const full = await api(`/api/courses/${encodeURIComponent(id)}`);
  DETAIL_CACHE[id] = full;
  return full;
}
...
async function openItem(id){
  ...
  curEl.innerHTML = `<div class="empty">Loading curriculum...</div>`;
  showView('item');

  let full;
  try {
    full = await fetchFullDetail(id);
  } catch (err) {
    curEl.innerHTML = `<div class="empty">Couldn't load curriculum. Try again.</div>`;
    return;
  }

  if (full.weeks && full.weeks.length) {
    curEl.innerHTML = full.weeks.map((w, idx) => `
      <details class="curriculum-week" ${idx===0 ? 'open' : ''}>
        <summary>${w.title}</summary>
        <ul>${w.lessons.map(l => {
          const title = typeof l === 'string' ? l : l.title;
          const preview = (typeof l === 'object' && l.content) ? `<div style="margin-top:4px; color:var(--muted); font-size:12.5px; line-height:1.5;">${l.content}</div>` : '';
          return `<li>${title}${preview}</li>`;
        }).join('')}</ul>
      </details>`).join('');
  } else {
  ...
  if (full.projects && (full.projects.mini || full.projects.major)) {
    curEl.innerHTML += `
      <h2 style="margin-top:28px;">Projects you'll build</h2>
      <p style="margin-bottom:16px;">Real projects for your portfolio — GitHub links go into your certificate.</p>
      <h3 style="margin-bottom:10px;">Mini projects</h3>
      <div class="grid" style="grid-template-columns:repeat(auto-fill,minmax(220px,1fr)); margin-bottom:20px;">
        ${full.projects.mini.map(p => `<div class="card" style="cursor:default;"><h3 style="font-size:14px;">${p.title}</h3><p style="font-size:12.5px; margin-top:6px;">${p.description}</p></div>`).join('')}
      </div>
      <h3 style="margin-bottom:10px;">Major projects</h3>
      <div class="grid" style="grid-template-columns:repeat(auto-fill,minmax(260px,1fr));">
        ${full.projects.major.map(p => `<div class="card" style="cursor:default; border-color:var(--amber);"><h3 style="font-size:14.5px;">${p.title}</h3><p style="font-size:12.5px; margin-top:6px;">${p.description}</p></div>`).join('')}
      </div>`;
  }
}
...
document.getElementById('signupForm').addEventListener('submit', async (e) => {
  ...
  try {
    const data = await api('/api/signup', { method:'POST', body: JSON.stringify({
      ...
      agreedToTerms: document.getElementById('suAgree').checked,
    })});
    setToken(data.token);
    await loadMe();
    showView('dashboard');
...
document.getElementById('forgotForm').addEventListener('submit', async (e) => {
  ...
  try {
    const data = await api('/api/forgot-password', { method:'POST', body: JSON.stringify({
      email: document.getElementById('forgotEmail').value,
    })});
    alertEl.innerHTML = `<div class="alert alert-ok">${data.message}</div>`;
  } catch (err) {
    alertEl.innerHTML = `<div class="alert alert-error">${err.message}</div>`;
  } finally {
...
document.getElementById('resetpwForm').addEventListener('submit', async (e) => {
  ...
  alertEl.innerHTML = '';
  if (!RESET_TOKEN) { alertEl.innerHTML = `<div class="alert alert-error">Missing reset token — please use the link from your email again.</div>`; return; }
  try {
    const data = await api('/api/reset-password', { method:'POST', body: JSON.stringify({
      token: RESET_TOKEN,
      newPassword: document.getElementById('resetpwNew').value,
    })});
    alertEl.innerHTML = `<div class="alert alert-ok">${data.message}</div>`;
...
function renderDashboard(){
  ...
  listEl.innerHTML = enrolled.map(e => {
    ...
    if (!item) return '';
    const totalLessons = item.totalModules || 1;
...
async function openLearn(courseId){
  ...
  const summary = ALL_ITEMS.find(i => i.id === courseId);
  if (!summary) return;
  learnActiveWeekIdx = 0;
  document.getElementById('learnType').textContent = summary.type;
  document.getElementById('learnTitle').textContent = summary.name;
  document.getElementById('learnWeekList').innerHTML = `<div class="empty">Loading...</div>`;
  document.getElementById('learnLessonList').innerHTML = '';
  showView('learn');

  let full;
  try {
    full = await fetchFullDetail(courseId);
  } catch (err) {
    document.getElementById('learnWeekList').innerHTML = `<div class="empty">Couldn't load content. Try again.</div>`;
    return;
  }
  currentItem = full;
  renderLearnSidebar(full, enrollment);
  renderLearnWeek(full, enrollment, 0);
}
...
function renderLearnSidebar(item, enrollment){
  ...
  const projectsMilestone = document.querySelector('.learn-milestone[data-milestone="projects"]');
  if (projectsMilestone) projectsMilestone.style.display = (item.projects && (item.projects.mini || item.projects.major)) ? 'flex' : 'none';

  const offerMilestone = document.querySelector('.learn-milestone[data-milestone="offer"]');
  if (offerMilestone) offerMilestone.style.display = item.type === 'internship' ? 'flex' : 'none';
  ...
  $all('.learn-milestone').forEach(el => {
    el.onclick = () => {
      ...
      if (el.dataset.milestone === 'projects') openItem(item.id);
    };
  });
}
...
function renderLearnWeek(item, enrollment, weekIdx){
  ...
  document.getElementById('learnCurrentLesson').textContent = typeof firstLesson === 'string' ? firstLesson : (firstLesson ? firstLesson.title : 'Lesson');
  document.getElementById('learnTheoryContent').style.display = 'none';
  document.getElementById('learnVideoPlaceholder').style.display = 'flex';
  ...
  document.getElementById('learnLessonList').innerHTML = week.lessons.map((lesson, li) => {
    ...
    return `<div class="lesson-item ${isDone?'done':''}" ${hasContent ? `data-read="${key}" style="cursor:pointer;"` : ''}>
      <span>${title}${hasContent ? ' <small style="color:var(--teal);">(click to read)</small>' : ''}</span>
      <button class="lesson-mark-btn ${isDone?'done':''}" data-mark="${key}">${isDone ? '✓ Done' : 'Mark complete'}</button>
    </div>`;
  }).join('');

  $all('[data-read]').forEach(el => {
    el.addEventListener('click', (e) => {
      if (e.target.closest('[data-mark]')) return; // don't trigger read when clicking the mark-complete button
      const key = el.dataset.read;
      const [wi, li] = key.split('-').map(Number);
      const lesson = item.weeks[wi].lessons[li];
      document.getElementById('learnVideoPlaceholder').style.display = 'none';
      const pane = document.getElementById('learnTheoryContent');
      pane.style.display = 'block';
      pane.innerHTML = `<h3 style="margin-bottom:12px;">${lesson.title}</h3><p>${lesson.content}</p>`;
      if (typeof pane.scrollIntoView === 'function') pane.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
    });
  });
...
function openCertForm(item){
  ...
  if (!enrollment) {
    area.innerHTML = `<div class="alert alert-error">You need to enroll (and pay) for this ${item.type} before claiming a certificate.</div>`;
    showView('certificate');
    return;
  }
  const needsOfferLetterFirst = item.type === 'internship' && !enrollment.offerLetter;
  if (needsOfferLetterFirst) {
...
document.getElementById('adminBackupBtn').addEventListener('click', async () => {
  ...
  try {
    ...
    if (!res.ok) { const d = await res.json().catch(()=>({})); throw new Error(d.error || 'Backup failed'); }
    const blob = await res.blob();
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url; a.download = `byteforge-backup-${new Date().toISOString().slice(0,10)}.json`;
    document.body.appendChild(a); a.click(); a.remove();
    URL.revokeObjectURL(url);
  } catch (err) {
...
/* Auto-open the reset-password form if arriving from an emailed reset link: /?reset=TOKEN */
