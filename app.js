...
function openItem(id){
  ...
  if (item.projects && (item.projects.mini || item.projects.major)) {
    curEl.innerHTML += `
      ...
      <div class="grid" style="grid-template-columns:repeat(auto-fill,minmax(220px,1fr)); margin-bottom:20px;">
        ${item.projects.mini.map(p => `<div class="card" style="cursor:default;"><h3 style="font-size:14px;">${p.title}</h3><p style="font-size:12.5px; margin-top:6px;">${p.description}</p></div>`).join('')}
      </div>
      <h3 style="margin-bottom:10px;">Major projects</h3>
      <div class="grid" style="grid-template-columns:repeat(auto-fill,minmax(260px,1fr));">
        ${item.projects.major.map(p => `<div class="card" style="cursor:default; border-color:var(--amber);"><h3 style="font-size:14.5px;">${p.title}</h3><p style="font-size:12.5px; margin-top:6px;">${p.description}</p></div>`).join('')}
      </div>`;
  }
  showView('item');
}
