...
async function generateOfferLetterPDF(data) {
  ...
  const sigY = doc.y;
  doc.font('Helvetica-Oblique').fontSize(13).fillColor('#0F1219').text(data.signature || data.studentName, 50, sigY, { width: 240 });
  doc.moveTo(50, doc.y + 4).lineTo(290, doc.y + 4).strokeColor('#999999').stroke();
  doc.font('Helvetica').fontSize(8.5).fillColor('#999999').text('Digitally signed by intern', 50, doc.y + 8, { width: 240 });

  doc.font('Helvetica').fontSize(10).fillColor('#333333').text('Sincerely,', 320, sigY, { width: 225 });
  doc.text(`${COMPANY_NAME} Team`, 320, doc.y, { width: 225 });
...
async function generateCertificatePDF(data) {
  ...
  const footerY = doc.page.height - 165;
  doc.image(qrBuffer, doc.page.width - 175, footerY - 5, { width: 85 });
  doc.fontSize(7.5).fillColor('#999999').text('Scan to verify', doc.page.width - 175, footerY + 83, { width: 85, align: 'center' });

  doc.fontSize(9).fillColor('#333333').text(`Certificate ID: ${data.certId}`, 70, footerY + 8);
  doc.text(`Register No: ${data.regNo}`, 70, footerY + 23);
  doc.text(`Issued: ${data.issuedDate}`, 70, footerY + 38);
  doc.fontSize(8).fillColor('#999999').font('Helvetica-Oblique').text(`Digitally signed & accepted by: ${data.signature || data.studentName}`, 70, footerY + 53, { width: 300 });

  doc.moveTo(70, footerY + 71).lineTo(260, footerY + 71).strokeColor('#999999').stroke();
  doc.fontSize(9).font('Helvetica').fillColor('#333333').text(`${COMPANY_NAME} Team`, 70, footerY + 76);
  doc.fontSize(7.5).fillColor('#999999').text('Authorized Signatory', 70, footerY + 89);
