...
async function generateCertificatePDF(data) {
  ...
  // Project portfolio links — internships only, proof of real work done
  if (data.githubMajor || data.githubMinor || data.linkedinPost) {
    y = doc.y + 18;
    doc.fontSize(9).font('Helvetica-Bold').fillColor(MUTED).text('PROJECT PORTFOLIO', 55, y, { width: W - 110, align: 'center', characterSpacing: 1 });
    y = doc.y + 6;
    const linkLines = [];
    if (data.githubMajor) linkLines.push(`Major Project (GitHub): ${data.githubMajor}`);
    if (data.githubMinor) linkLines.push(`Mini Project (GitHub): ${data.githubMinor}`);
    if (data.linkedinPost) linkLines.push(`Project Showcase (LinkedIn): ${data.linkedinPost}`);
    doc.fontSize(8).font('Helvetica').fillColor('#2A2E36').text(linkLines.join('   \u00b7   '), 70, y, { width: W - 140, align: 'center' });
  }
